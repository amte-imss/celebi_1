 ...
 * */
class MY_Controller extends CI_Controller implements Accesos_modulos, Gestion_files
{

    private $accesos_disponibles = array();
...
//            set_datos_imagen_perfil
    ...
    {
        $data_usuario = $this->session->userdata('control_escolar')['usuario'];
...
//                    }
     ...
     * @author LEAS
     * @param type POST: {
     * tipo_help: {1->por opciones}
     * entidad: (nombre_completo)
     * seleccion:{opciones de secciones}
     * formato:{view_formato}
     * }
     */
    public function helper_catalogos()
    {
        if ($this->input->post())
        {
            $datos_post = $this->input->post(true, null);
            switch ($datos_post['tipo_help'])
            {
                default ://ayudas por opción
            }
        }

        $this->load->library('Ayudas_textos');
        $result = $this->ayudas_textos->encrypt_catalogo('hola');
//        pr($result);
        $result_2 = $this->ayudas_textos->decrypt_catalogo($result);
//        pr($result_2);
    }

    public function get_accesos()
    {
        if (empty($this->accesos_disponibles))
        {
            $this->set_accesos(); //Carga modulos disponibles
        }
        return $this->accesos_disponibles;
    }

    public function is_acceso($niveles_requeridos = array(), $niveles_disponibles = array())
    {
        $acceso = $this->lniveles_acceso->nivel_acceso_valido($niveles_requeridos, $niveles_disponibles);
        return $acceso;
    }

    public function set_accesos($accesos = null)
    {
...
include_once APPPATH . 'core/Control_escolar.php';
