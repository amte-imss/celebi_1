...
class Implementaciones_model extends MY_Model
{
    ...
    public function get_participantes($filtros = null)
    {
        $this->db->flush_cache();
        $this->db->reset_query();

        $select = array('r.nombre rol', 'i.*');
        $this->db->select($select);
        $this->db->join('presenciales.rol_participante r', 'i.id_rol_participante = r.id_rol_participante', 'inner');
        ...
        $this->db->where(array('i.activo' => true));

        $res = $this->db->get('presenciales.participantes_implementacion i');
        $this->db->flush_cache();
        $this->db->reset_query();
    ...
    public function get_listado_implementacion($filtros = null)
    {
        ...
        $select = [
            "i.clave_curso", "i.id_implementacion", "i.clave_implementacion", "c.nombre nombre_curso"
    ...
    public function get_implementacion($filtros)
    {
        ...
        $select = [
            ...
            , "i.horas_aula", "i.horas_extra", "e.cve_estado_implementacion"
...
//                $id = $this->db->insert_id();
    ...
    public function insert_implementacion_step1($datos, $convocatoria)
    {
        $result = 0;
        $this->db->trans_begin();

        $this->db->insert('presenciales.implementacion', $datos);
        if ($this->db->trans_status() === FALSE)
        {
            $this->db->trans_rollback();
        } else
        {
            $implementacion = $this->db->insert_id();
            $historico = ['id_implementacion' => $implementacion, 'cve_estado_implementacion' => 'RE'];
            $this->db->insert('presenciales.historico_estado_implementacion', $historico);
            if ($this->db->trans_status() === FALSE)
            {
                $this->db->trans_rollback();
            } else
    ...
    public function update_status_implementacion($datos)
    {
        $result = 0;
        $this->db->trans_begin();
        $this->db->where('id_implementacion', $datos['id_implementacion']);
