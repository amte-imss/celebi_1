...
class Usuario extends MY_Controller
{
    ...
    const LIMIT = 10, LISTA = 'lista', BASICOS = 'basico', PASSWORD = 'password',
            NIVELES_ACCESO = 'niveles', STATUS_ACTIVIDAD = 'actividad', CENSO = 'censo';
    ...
    public function get_usuarios($usuario = Usuario_model::LISTA, $limit = Usuario_model::LIMIT, $order_by = 'asc', $page_number = 0)
    {
        ...
        $params = array(
            'select' => array(
                ...
                'E.id_unidad_instituto', 'usuarios.censo_finalizado'
            )
        );
...
//        $this->output->enable_profiler(true);
        ...
        if ($this->input->post() && $this->input->is_ajax_request())
        {
            ...
            switch ($tipo)
            {
                ...
                case Usuario::STATUS_ACTIVIDAD:
                    ...
                    break;
                case Usuario::CENSO:
                    $validations = $this->config->item('form_usuario_censo_finalizado');
                    break;
            }
            ...
            if ($this->form_validation->run() == TRUE)
            {
                ...
                switch ($tipo)
                {
                    ...
                    case Usuario_model::NIVELES_ACCESO:
                        ...
                        break;
                    case Usuario::CENSO:
                        $view = $this->get_censo_finalizado($id_usuario);
                        break;
                }
            } else
            {
    ...
    private function get_censo_finalizado($id_usuario = 0)
    {
        $params['where'] = array(
            'usuarios.id_usuario' => $id_usuario
        );
        $params['select'] = array(
            'usuarios.id_usuario', 'usuarios.censo_finalizado'
        );
        $resultado = $this->usuario->get_usuarios($params);
        $output['id_usuario'] = $id_usuario;
        $output['usuario'] = $resultado;
        return $this->load->view('usuario/censo_finalizado.tpl.php', $output, true);
    }
    ...
    public function exportar()
    {
        ...
        header("Pragma: no-cache");
        header("Expires: 0");
        echo $this->load->view('usuario/tabla_usuarios_xls.tpl.php', $output, TRUE);
        exit();
    }

    public function finalizar_censo()
    {

        $datos_sesion = $this->get_datos_sesion();
        $this->load->model('Modulo_model', 'modulo');
        $this->load->library('LNiveles_acceso');
        $niveles = $this->modulo->get_niveles_acceso($datos_sesion['id_usuario'], 'usuario');
        $acceso = $this->lniveles_acceso->nivel_acceso_valido(array(LNiveles_acceso::N1, LNiveles_acceso::Nivel_central), $niveles);
//        pr("entra");
//        pr(($acceso) ? 1 : 0);
        if ($acceso)
        {
            if (isset($datos_sesion['id_usuario']) && !empty($datos_sesion['id_usuario']) && $datos_sesion['id_usuario'] > 1)
            {
                $params = array(
                    'where' => array('usuarios.id_usuario' => $datos_sesion['id_usuario']),
                    'select' => array(
                        'usuarios.id_usuario', 'docentes.matricula', 'docentes.id_docente', 'docentes.nombre',
                    )
                );
                $result = $this->usuario->update_finaliza_censo($params);
                if ($result)
        ...
        {
//            $cadena = $this->uri->rsegment(1) ;//. '/' . $this->uri->rsegment(2);
            $cadena = base_url();
            redirect($cadena);
        }
    }
